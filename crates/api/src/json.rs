...
//! this module additionally *parses*, since the API has to read whatever
//! JSON a hostile RL agent sends it, never just write JSON of its own.
//!
//! The parser never panics: every malformed byte sequence becomes
//! `Err(JsonError)`, which the HTTP layer turns into a 400 response with a
//! reason, exactly the same "never crash on hostile input" discipline
//! `archipelago_sim::action` already applies to `Action`.
...
impl Value {
    ...
    pub fn as_f32(&self) -> Option<f32> {
        self.as_f64().filter(|n| n.is_finite()).map(|n| n as f32)
    }
